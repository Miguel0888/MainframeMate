...
public class ExcelImportDialog extends JDialog {
    ...
    public ExcelImportDialog(Frame owner) {
        super(owner, "Excel-Import", true);
    ...
    private void loadSatzartenJson(File jsonFile) {
        try (Reader reader = new InputStreamReader(new FileInputStream(jsonFile), StandardCharsets.UTF_8)) {
            ...
            getPluginSettings().put(KEY_JSON, jsonFile.getAbsolutePath());
    ...
    private void savePluginSettings() {
        Settings fullSettings = SettingsManager.load();
        Map<String, String> pluginSettings = fullSettings.pluginSettings.computeIfAbsent(PLUGIN_KEY, k -> new LinkedHashMap<>());

        if (selectedExcelFile != null) {
            pluginSettings.put(KEY_EXCEL, selectedExcelFile.getAbsolutePath());
        }

        if (trennzeileField.isEnabled()) {
            pluginSettings.put(KEY_TRENNZEILE, getTrennzeile());
        }

        fullSettings.pluginSettings.put(PLUGIN_KEY, pluginSettings);
