...
public class BrowseSnapshotTool implements McpServerTool {
    ...
    private String buildDescribeScript(String scopeExpr) {
        return "(function(){"
             ...
             + "}}catch(e){}"
             + "var idx=0;"
             + "for(var i=0;i<allNodes.length&&idx<200;i++){"
             + "  var n=allNodes[i];"
             // Better visibility check: offsetParent is null for fixed/sticky elements
             // Use getComputedStyle instead to check display/visibility
             + "  try{var cs=window.getComputedStyle(n);if(cs.display==='none'||cs.visibility==='hidden')continue;"
             + "  if(n.offsetWidth===0&&n.offsetHeight===0&&n.tagName!=='BODY'&&n.tagName!=='HTML')continue;}catch(e){continue;}"
             ...
             + "  var hr='';"
             + "  try{var rawHr=n.getAttribute('href')||'';"
             + "  if(tag==='a'&&n.href){hr=n.href;}"
             ...
             // Fallback: if tx is empty, try first child text nodes
             + "  if(!tx){try{for(var ci=0;ci<n.childNodes.length&&ci<5;ci++){if(n.childNodes[ci].nodeType===3){tx=(n.childNodes[ci].textContent||'').trim().substring(0,50);if(tx)break;}}}catch(e){}}"
             ...
             + "  if(hr&&hr.indexOf('javascript:')<0)desc+=' ->'+(hr.length>200?hr.substring(0,200):hr);"
